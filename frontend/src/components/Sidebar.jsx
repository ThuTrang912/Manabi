    ...
    <div className="fixed inset-0" style={{ zIndex: 2000 }}>
      ...
      >
        ...
        <div
          ...
          onClick={() => {
            setShowSidebar(false);
            if (navigate) navigate('/folders/new');
          }}
        >
        ...
        <div
          ...
          onClick={() => {
            setShowSidebar(false);
            if (navigate) navigate('/dictionary');
          }}
        >
          <span className="material-icons">library_books</span> Từ điển
        </div>
        <div
          className="text-gray-700 flex items-center gap-2 cursor-pointer hover:bg-gray-100 rounded px-2 py-1"
          onClick={() => {
            setShowSidebar(false);
            if (navigate) navigate('/expert-solutions');
          }}
        >
          <span className="material-icons">question_answer</span> Lời giải chuyên gia
        </div>
      </aside>
    </div>
  );
